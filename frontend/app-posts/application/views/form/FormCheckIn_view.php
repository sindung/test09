...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        table = $('#table').DataTable({
            ...
            "ajax": {
                "url": "<?= site_url('FormCheckIn/ajax_list') ?>",
                "type": "POST"
            },
    ...
    function ubah_data(id)
    {
        ...
        $.ajax({
            url: "<?php echo site_url('FormCheckIn/ajax_edit') ?>/" + id,
            type: "GET",
            dataType: "JSON",
    ...
    function save()
    {
        ...
        if (save_method === 'add') {
            url = "<?php echo site_url('FormCheckIn/ajax_add') ?>";
        } else {
            url = "<?php echo site_url('FormCheckIn/ajax_update') ?>";
        }
    ...
    function hapus_data(id)
    {
        if (confirm('Are you sure delete this data?'))
        {
            ...
            $.ajax({
                url: "<?php echo site_url('FormCheckIn/ajax_delete') ?>/" + id,
                type: "POST",
                dataType: "JSON",
                success: function (data)
                {
        ...
        if (list_id.length > 0)
        {
            if (confirm('Are you sure delete this ' + list_id.length + ' data?'))
            {
                $.ajax({
                    type: "POST",
                    data: {id: list_id},
                    url: "<?php echo site_url('FormCheckIn/ajax_bulk_delete') ?>",
                    dataType: "JSON",
                    success: function (data)
                    {
                        if (data.status)
