...
class FormCheckOut extends CI_Controller {

    public function index() {
        $array = array();

        $data = array(
            'app_title' => 'App Parkir - Form Check Out',
            'app_heading' => 'Form Check Out',
            'app_content' => $this->parser->parse('form/FormCheckOut_view', $array, true)
        );
    ...
    public function ajax_list_by($nomor_polisi) {
        $this->load->helper('url');

        $list_in = $this->model_FormCheckIn->get_by_nomor_polisi($nomor_polisi);
//        $list_out = $this->model_FormCheckOut->get_by_nomor_polisi($nomor_polisi);

        /*
         * Ketentuan biaya parkir :
            - 1 jam pertama = Rp5,000
            - Tiap jam berikut = Rp4,000
            - Maksimal 1 hari (24jam) = Rp50,000
         */
        
        $data = array(
            "nomor_polisi" => $list_in->nomor_polisi,
            "tanggal" => date_format(date_create($list_in->tanggal), "Y-m-d"),
            "jam" => date_format(date_create($list_in->jam), "H:i:s"),
            "jenis_kendaraan" => $list_in->jenis_kendaraan,
            "biaya_parkir" => 5000
        );
